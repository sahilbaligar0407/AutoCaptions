#!/usr/bin/env python3
"""
Minimal FFmpeg Test for Progressive Captions
This script tests FFmpeg rendering with just one caption state
"""
...
def test_minimal_ffmpeg():
    """Test FFmpeg with minimal captions"""
    
    print("=== Minimal FFmpeg Test ===")
    ...
    # Take only first segment for testing
    ...
    # Filter to only first 1 state for testing
    test_states = [s for s in states if not s.skip][:1]
    print(f"Generated {len(test_states)} test caption states")
    
    # Generate FFmpeg filter
    ffmpeg_gen = FFmpegGenerator("Poppins-Black.ttf")
    filter_script = "temp/minimal_test_filter.txt"
    ...
    print(f"Filter length: {len(filter_content)} characters")
    
    # Test FFmpeg command
    video_input = "ClipV1.mp4"
    output_file = "outputs/minimal_test_output.mp4"
